...
def test_api_missing_field():
    ...
    assert r.json() == {
        ...
        'detail': [
            {'loc': ['body', 'Age'], 'msg': 'Field required', 'input': {'Name': 'Bauer, Sir. John', 'Fare': 5.0}},
            {'loc': ['body', 'Pclass'], 'msg': 'Field required', 'input': {'Name': 'Bauer, Sir. John', 'Fare': 5.0}}
        ]
    }
...
def test_api_invalid_value():
    ...
    assert r.json() == {
        'error': 'Schema Validation error',
        'detail': [{'loc': ['body', 'Age'], 'msg': 'Value error, age must >=0', 'input': -10},
                   {'loc': ['body', 'Pclass'], 'msg': 'Value error, Pclass must be 1, 2, or 3', 'input': 5},
                   {'loc': ['body', 'Fare'], 'msg': 'Value error, Fare must be positive', 'input': 0}
                   ]
    }
...
def test_api_invalid_type():
    ...
    assert r.json() == {
        'error': 'Schema Validation error',
        'detail': [
            {'loc': ['body', 'Pclass'], 'msg': 'Value error, Pclass must be 1, 2, or 3', 'input': 'first'}
        ]
    }
...
def test_api_empty_field():
    ...
    assert r.json() == {
        'error': 'Schema Validation error',
        'detail': [
            {'loc': ['body', 'Name'], 'msg': 'Value error, Name must not be blank', 'input': None}
        ]}
